...
(defun procura-A-iter  (problema heuristica)
  (let ((estados-por-analisar (list))
        (predecessores (make-hash-table :size 1000))
        (f_n (make-hash-table :size 1000))
        (novo-estado)
        ...
        (loop while (and (not solucao) (not (null estados-por-analisar)))
          ...
          (cond ((funcall (problema-solucao problema) estado-actual)
                ...
                (dolist (accao accoes)
                  (setf novo-estado (funcall resultado estado-actual accao))
                  (setf (gethash novo-estado f_n) (+ (funcall custo-caminho novo-estado) (funcall heuristica novo-estado)))
                  (setf estados-por-analisar (lista-insere-ordenada-iter novo-estado estados-por-analisar f_n (gethash novo-estado f_n)))
                  (insere-estado-info novo-estado (cria-estado-info :estado-anterior estado-actual :accao accao) predecessores)))
...
(defun procura-A*3 (problema heuristica)
  (let ((estados-por-analisar (list))
        (f_n (make-hash-table :size 1000))
        (novo-estado)
        (accoes (list))
...
(defun procura-best2 (array lista-pecas)
  (let ((problema)
        (estado-inicial))
...
(defun procura-best (array lista-pecas)
  (let ((problema)
    ...
    (procura-A* problema #'irregularidades))) 
...
(defun lista-insere-ordenada-teste (elemento lista)
  (let ((nova-lista) (para nil))
  
    (loop while (and (not para) (not (null lista)))
      do
      (cond ((>= (first lista) elemento)
              (push elemento lista)
              (setf nova-lista (append nova-lista lista))
              (setf para t)
            )
...
(defun superheuristica (estado-pesos)
  #'(lambda (estado) (+   (* (aref estado-pesos 0) (irregularidades estado)) 
                          ...
                          (* (aref estado-pesos 4) (buracos-totais estado))
                          (* (aref estado-pesos 5) (espacos-livres-totais estado)))))
        
      

(defun faz-ate-encontrar()
  (let ((sucessornum (sucessor2 #(1 4 5 2 1 1))))
  (loop while (< (avaliacao sucessornum) 600) do (print (avaliacao sucessornum)) (print sucessornum) (setf sucessornum (sucessor2 sucessornum)))
  sucessornum))
...
(defun sucessor2 (estado)
  (let  ((novo-estado (make-array 6)))
        
    (dotimes (elemento 6 novo-estado)
      (setf (aref novo-estado elemento) (random 100.0))
    )
  )
)
...
(defun teste (numero-tabuleiros numero-pecas heuristica nome-ficheiro)
  (let  ((tabuleiro)
        (lista-pecas)
        (estado-final)
        (output))
    ...
    (dotimes (tabuleiro-n numero-tabuleiros)
      ...
      (NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)))
        ...
        (setf lista-pecas (random-pecas numero-pecas))
        (setf estado-final (procura-best2 (tabuleiro->array tabuleiro) lista-pecas))
...
(defun teste2 (heuristica)
  ;teste (4 #'espacos-livres-totais "espacos-livres-totais") )
   (let  ((tabuleiro)
       (lista-pecas)
         (estado-final)
         (output)
